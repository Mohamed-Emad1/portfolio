...
class PortfolioManager {
    ...
    createProjectCard(project, index) {
        ...
        const statusBadge = project.featured ? '<span class="featured-badge">Featured</span>' : '';
        const statusIndicator = project.status ? `<span class="status-badge status-${project.status.toLowerCase().replace(' ', '-')}">${project.status}</span>` : '';
